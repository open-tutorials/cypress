...
<details>
  ...
  <p>Создайте файл `test.js` с содержимым `console.log(new Date().toISOString());`</p>

  <p>Потом выполните команду `node test.js`</p>
...
it('открыть сайт', () => { cy.visit('https://www.cypress.io/'); });
