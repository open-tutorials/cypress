...
<details>
  ...
  Потом в `Терминале` выполните команду `node test.js`
...
<details>
  ...
  Можно просто выполнить команду `npm init -y` и файл `package.json` будет создан сразу.
...
Некоторые тестовые задания скрываются под `Basic` авторизаций. Например: https://finance.dev.fabrique.studio/accounts/login/
