...
* Ты инициализировал чистый **Node.js** проект
  - [ ] Создай папку проекта `%/projects/cypress/best_selectors`
  - [ ] Открой папку проекта в Visual Code
  - [ ] Выполни в терминале команду `npm init -y`
...
- [ ] Установи через терминал пакет `npm install tiny-server`
...
- [ ] Установи через терминал пакет `npm install node-wget`
- [ ] Для удобства в `package.json` добавь команду `wget` в раздел `scripts`:
```json
...
  "scripts": {
    "start": "tiny-server",
...
- [ ] В терминале выполни команду `npm start`
- [ ] Открой в **Chrome** URL http://localhost:3000
- [ ] В списке файлов открой `apps/tesla.html`
...
- [ ] Создай файл `cypress/integration/best-selectors.spec.js`
...
```
- [ ] 🔥 Не забудь отформатировать код, и сохранить файл!
- [ ] Открой **Терминал** → **Новый терминал**
- [ ] Запусти **Cypress** командой `npm run cypress open`
- [ ] В **Cypress** выбери тест `best-selectors.spec.js`
...
- [ ] Наведи на поле **фамилия** и скопируй код селектора.
- [ ] Обнови код теста:
...
- [ ] Через **selector playground** найди селекторы для:
...
- [ ] Нажми правой кнопкой мышки на любом элементе, например кнопке войти.
- [ ] Выбери из контекстного меню **Просмотр кода элемента**.
...
- [ ] Аналогично найди селекторы и заполни:
  * поле **Фамилия**
  * поле **Email**
...
- [ ] Найди **контекст** для всех полей формы обновите код теста.
- [ ] Сверь свой исходный код с [примером](cypress/integration/best-selectors.spec.js)

*****

## 7. Тест формы входа

- [ ] Напиши **тест кейс** **вход по email** в **тест сьюитe** **Вход в приложение**.
- [ ] Сверь свой исходный код с [примером](cypress/integration/best-selectors.spec.js)
...
- [ ] Замени селектор `cy.get('form.login [type=submit]')` на `cy.contains('Войти')`
- [ ] Переключи язык приложения добавлением параметра `lang=en` в URL:
```javascript
