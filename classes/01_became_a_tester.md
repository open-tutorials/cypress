...
> 
> * Нет ли ошибок в тесте? 
> * Все знаки припенания расставлены правильно? 
...
> 
> * Программа не компроментирует пользователя?
> * Не причиняет боль и не травмирует ?
...
> * На разных устройствах: ноутбку, смартфон, планшет?
...
Мы отправли разработчикам список багов, они их исправили и выпустили версию `1.1`
...
> 
> Только тогда он сможет находить в них **ошибки!**
...
> Перезагруска страницы перезапускает приложение в браузере.
...
POST http://localhost:3000/api/login &rarr; 404
...
<details>
  ...
  * 🐞 После отправки денег баланс текущего пользователя не обновляется.
...
Воспользуйтесь `тулзой`  [SQL Light Viewer](https://inloop.github.io/sqlite-viewer/) для просмотра содержимого файла базы.

❓ Какие там таблицы и поля? За что они отвечают?

## 6. Сессия пользователя и `Печеньки`

На самом деле в приложении есть еще один большой баг.
...
Как же `Бекенд` понимает, что запрос ему делает `chester`, а не `bob`?

Зайдите в **Инструментах разработчика** в `Приложение` &rarr; `Cookies`

Нашли `Печеньку` с именем `session`? 

Скопируйте это значение, например `xxx`
...
❓ А где баг на Фронтенде или Бекенд? Запрос на сервер вообще делаться?

# 🤩 Хочу еще

Повторите туториал еще раз сами. Выпишите список вопросов.

Задайте вопросы в https://chat.epic1h.com в канале `#qa_lean`

Попробуйте исследовать приложение `Сбербанк Онлайн` в браузере.

Составьте список багов.

# Термины
* HTML — язык разметки гипер-текста.
* CSS — язык оформления гипер-текста.
* UI / User Interface — как выглядит пользовательский интерфейс программы.
* UX / User Experience — насколько удобство использовать интерфейс программы.
...
* Сервер / Server — компьютер который `отдает` информацию через Интернет.
* Клиент / Client — компьютер который `получает` информацию через Интернет.
* Веб-сервер / WEB server — программа запущеная на `Сервере` для `обработки` запросов от `Браузера`
* Браузер / Browser -  программа запущеная на `Клиенте` для `отправки` запросов `Веб-серверу`
* Фронтенд / Front-end — часть нашей программы запущенная на `Веб-сервере`
* Бекенд / Back-end — часть нашей программы запущенная в `Браузере`
