...
> 
> Интерфейс не компроментирует пользователя?
...
А теперь откройте приложение по ссылке https://qa.epic1h.com/?v=1.1 и проверьте исправления.
...
> Тестировщик — понимает как разрабатываются программы и как они работают.
...
Кто удалил кнопку из приложения? И как теперь работать? Все хорошо, просто **перезагрузите** страницу.
...
* Загружает данные с бекенда и показывает пользователю.
* Собирает и отправляет данные на бекенд.

#### Что делает `Бекенд`
* Загружает данные из `БД` и передает фронтенду.
* Обрабатывает данные от фронтенда и записывает их в `БД`
* Проверят права доступа к данным в `БД`
...
Разработчики предоставили нам для целей тестирования доступ в БД на Бекенде.
...
Переведите запрос `SELECT * FROM users` на русский язык.
...
Найдите в нем баги! Что делать?
* Переведите от `chester` к `bob` 100 рублей.
* Выйдите и зайдите под `bob` и проверьте баланс.
...
Разработчики предоставили нам для целей тестирования доступ к логам на сервере.

Откройте в новой вкладке о https://qa.epic1h.com/server.log

Переведите на русский эти записи.
