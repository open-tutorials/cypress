...
Открываем в браузере `Веб-приложение` https://qa.epic1h.com/
...
Как же Бекенд понимает, что запрос ему делает `chester` или `bob`?

Зайдите в **Инструментах разработчика** `Приложение` &rarr; `Cookies`

Нашли `Печеньку` с именем `session`? Скопируйте это значение, например `xxx`

А теперь выполните запрос `SELECT * FROM sessions WHERE id = 'xxx'`
