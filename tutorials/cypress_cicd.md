...
Задача подготовить набор сервисов (контейнеров) проекта для деплоя на любом сервере.
...
- [x] В новом терминале **запусти** все сервисы:
...
- [x] Проверь, что тест провален **в контейнере cypress!**
...
- [x] Обнови хуки `before` файлов тестов:
...
❓ Что такое RPC?
❓ Гд теперь выполняются команды сброса и заполнения БД?
...
- [x] Добавь все файлы для фиксации изменений:
...
- [x] Отправить изменения на удаленный репозиторий на GitHub:
...
- [x] Найди и джобу на GitHub.
- [x] Дождись выполнения и сообщения в ошибке в локах.
- [x] Проверь сообщение на почте об ошибке (может прийти с задержкой ~10 минут).
- [x] Скачай артефакты задачи.
- [x] Посмотри видео прогона теста на GitHub.
