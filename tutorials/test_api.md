...
> 
> Но Open API это в первую очередь – **JSON схема** описания API.
...
## +7. Тесты регистрации пользователя
...
it.only('should not do register user with empty data', () => {
    ...
    cy.request({ method: 'POST', url: '/users', payload, failOnStatusCode: false })
        .then(({ status, body }) => {
...
**Что, если бы ты мог:**
* пройти авторизацию 1 раз перед стартом через API,
...
- [x] Войди на фронтенд https://demo.realworld.io/
- [x] Найди данный заголовок в XHR запросах.
- [x] Напиши тест для ендпойнта `GET /api/user`
