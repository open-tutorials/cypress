...
* Опишем тест-кейсы в Markdown для добавления, удаляения и редактирования в приложении блогов.
* Разберем и напишем оптимальный код с хуками, алиасами, функциями и т.д.

# 🙋‍ Перед началом

* Ты инициализировал чистый **Node.js** проект `%/projects/cypress/write_more_tests`
...
**Задача: составить список функциональных тестов.**
...
* ❓ Чем отличается `* [ ] ...` от `* [x] ...`?
* ❓ Почему в тест-сьютие **Sign up** все тесты `[x]`?
...
Например, добавить дату или случайное число:
...
- [x] Создай фейковых пользователей:
...
- [x] Создай файл `~/cypress/support/shared.js` из [примера](../cypress/support/write_more_tests/shared.js)
...
- [x] Сверь свой код с [примером](../cypress/integration/write_more_tests/articles1.spec.js)
...
- [x] Что бы избежать дублирования кода, создай функцию `addArtile() → article`
...
- [x] Напиши тест удаления статьи следуя описания тест-кейса:
  * Используй функцию `addArtile()`
  * Что бы найти добавленную статью используй сниппет:

```js
const article = addArtile();
...
```
  ...
  * Что бы опредить, что статья удалена:
...
- [x] Сверь свой код с [примером](../cypress/integration/write_more_tests/articles2.spec.js)
...
- [x] Сверь свой код с [примером](../cypress/integration/write_more_tests/articles3.spec.js)
- [x] Добавь отметку в `README.md` что тест готов.
